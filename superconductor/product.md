...
## Vision
Superconductor is a Gemini CLI extension that enables **Context-Driven Development**, turning the Gemini CLI into a proactive project manager that follows a strict protocol to specify, plan, and implement software. It aims to supercharge the development lifecycle by incorporating "superpowers" and advanced tool use, such as the `superconductor-kernel` MCP server for code intelligence, graph queries, and vetted components, plus expert `code review` skills to ensure all code meets rigorous project standards.
...
4. **Leverage Vetted Components:** Integrate with `superconductor-kernel` to utilize a library of high-quality, pre-tested UI components, code intelligence graph queries, and logic.
