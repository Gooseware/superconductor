...
4. **Final Review Retention & Swarm Feeding:** The `--fast` flag must NOT bypass the final Oracle review. When `--fast` is NOT provided, the Oracle must be forced to use the "Review Panel Mode" (where the heterogeneous review swarm feeds findings into the final Oracle Arbiter). When `--fast` is provided, it may downgrade to the Monolithic Oracle.
