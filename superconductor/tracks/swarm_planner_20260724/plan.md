...
- [ ] Task: Implement `ModelTierRouter` in `packages/superconductor-core/src/intelligence/model-tier-router.ts`. [TIER-3] [AGENT:caduceus-processor]
    - [ ] Define `ModelTier` type: `'flash-lite' | 'flash' | 'pro' | 'pro-thinking'`
    - [ ] Implement `route(tcs: TaskComplexityScore): ModelTier` mapping:
        - total 0–5 → `flash-lite`
        - total 6–10 → `flash`
        - total 11–15 → `pro`
        - total 16–20 → `pro-thinking`
    - [ ] Implement `formatAnnotation(tcs: TaskComplexityScore): string` → produces `[TIER-N:TCS=<total>]` string for plan injection
    - [ ] Export from `src/intelligence/index.ts`
- [ ] Task: Write unit tests for `ModelTierRouter` covering all band boundaries and annotation format. [TIER-3] [AGENT:caduceus-processor]
- [ ] Task: Superconductor - User Manual Verification 'Phase 2: Model Tier Router' (Protocol in workflow.md)

## Phase 3: Parallelism Optimiser & Wave Scheduler
- [ ] Task: Implement `ParallelismOptimiser` in `packages/superconductor-core/src/intelligence/parallelism-optimiser.ts`. [TIER-4] [AGENT:caduceus-oracle]
    - [ ] Define `SwarmWave`: `{ waveIndex, tasks: PlanTask[], models: ModelTier[], estimatedTokens: number, estimatedMinutes: number }`
    - [ ] Define `SwarmWaveSchedule`: `{ waves: SwarmWave[], totalTasks, totalEstimatedTokens, maxConcurrent }`
    - [ ] Implement DAG analysis: parse plan tasks for dependency markers (e.g. "requires", "after", sequential numbering) to build adjacency graph
    - [ ] Implement wave packing: assign tasks to waves respecting dependency ordering and `maxConcurrent` ceiling (default: 6)
    - [ ] Implement duration estimation: `flash-lite=1min/task, flash=2min/task, pro=5min/task, pro-thinking=10min/task`
    - [ ] Export from `src/intelligence/index.ts`
- [ ] Task: Write unit tests for `ParallelismOptimiser`. [TIER-3] [AGENT:caduceus-processor]
    - [ ] Test flat DAG (all parallel) → single wave or maxConcurrent-bounded waves
    - [ ] Test linear chain → one task per wave
    - [ ] Test mixed graph with 2 dependency chains
    - [ ] Test maxConcurrent cap enforcement
- [ ] Task: Superconductor - User Manual Verification 'Phase 3: Parallelism Optimiser' (Protocol in workflow.md)

## Phase 4: Token Budget Estimator
- [ ] Task: Implement `TokenBudgetEstimator` in `packages/superconductor-core/src/telemetry/token-budget-estimator.ts`. [TIER-3] [AGENT:caduceus-processor]
    - [ ] Define `TokenBudgetEstimate`: `{ contextTokens, reasoningTokens, outputTokens, reviewTokens, totalEstimate }`
    - [ ] Implement `estimate(task: PlanTask, tier: ModelTier): TokenBudgetEstimate` using heuristics:
    ...
    - [ ] Implement `estimateTrack(waves: SwarmWaveSchedule): TrackTokenBudget` with human-readable summary
    - [ ] Implement `formatCostEstimate(budget: TrackTokenBudget): string` → `"~4.2M tokens · ~$0.84 at Flash rates"`
        - Flash-lite rate: $0.075/1M tokens; Flash: $0.15/1M; Pro: $3.50/1M; Pro-thinking: $10/1M
    - [ ] Export from `src/telemetry/index.ts`
- [ ] Task: Write unit tests for `TokenBudgetEstimator`. [TIER-3] [AGENT:caduceus-processor]
    - [ ] Test each tier's token estimate formula
    - [ ] Test cost formatting output for known inputs
    - [ ] Boundary test: empty plan (0 tasks) → $0.00
- [ ] Task: Superconductor - User Manual Verification 'Phase 4: Token Budget Estimator' (Protocol in workflow.md)

## Phase 5: Oracle Cadence Optimiser
- [ ] Task: Implement `OracleCadenceOptimiser` in `packages/superconductor-core/src/intelligence/oracle-cadence-optimiser.ts`. [TIER-3] [AGENT:caduceus-processor]
    - [ ] Implement `compute(taskCount: number, avgTCS: number, historicalRetryRate?: number): number` returning oracle cadence:
    ...
    - [ ] Export from `src/intelligence/index.ts`
- [ ] Task: Write unit tests for `OracleCadenceOptimiser`. [TIER-3] [AGENT:caduceus-processor]
    - [ ] Test: 10 tasks, avgTCS=5 → expected cadence
    - [ ] Test: 4 tasks, avgTCS=15 (high complexity) → cadence ≤ 2
    - [ ] Test: high retry rate modifier reduces cadence
    - [ ] Test: cadence never returns < 1
- [ ] Task: Superconductor - User Manual Verification 'Phase 5: Oracle Cadence Optimiser' (Protocol in workflow.md)

## Phase 6: Swarm Blueprint Generator
- [ ] Task: Implement `SwarmBlueprintGenerator` in `packages/superconductor-core/src/intelligence/swarm-blueprint-generator.ts`. [TIER-4] [AGENT:caduceus-oracle]
    - [ ] Orchestrate: TaskComplexityScorer → ModelTierRouter → ParallelismOptimiser → TokenBudgetEstimator → OracleCadenceOptimiser
    - [ ] Implement `generate(planMarkdown: string): SwarmBlueprint` parsing plan tasks and producing the full blueprint
    - [ ] Implement `formatBlueprintSection(blueprint: SwarmBlueprint): string` producing the `## Swarm Blueprint` markdown table block for injection into `plan.md`
    - [ ] Implement `annotatePlan(planMarkdown: string): string` — returns plan with `[TIER-N:TCS=<score>]` annotations replacing static `[TIER-N]` on each task line
    - [ ] Export from `src/intelligence/index.ts`
- [ ] Task: Write integration tests for `SwarmBlueprintGenerator` using realistic plan inputs from existing tracks. [TIER-3] [AGENT:caduceus-processor]
    - [ ] Test with `token_estimation_20260723/plan.md` as input — verify blueprint is produced
    - [ ] Test annotation replacement preserves all plan content except the TIER annotation
    - [ ] Test with single-task plan (edge case)
