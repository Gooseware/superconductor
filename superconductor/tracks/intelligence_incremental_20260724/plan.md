...
- [ ] Task: Update `runComplexity` signature to accept `scopedFiles?: string[]`. [TIER-3:TCS=6] [AGENT:caduceus-processor]
...
- [ ] Task: Update `runDependencyGraph` to accept `scopedFiles?: string[]`. [TIER-3:TCS=7] [AGENT:caduceus-processor]
...
- [ ] Task: Update `runSast`, `runSymbolExtraction`, `runTestGaps`, `runPackageSurface` to accept `scopedFiles?: string[]`. [TIER-3:TCS=5] [AGENT:caduceus-processor]
...
- [ ] Task: Write unit tests for all 6 scoped runner variants. [TIER-3:TCS=5] [AGENT:caduceus-processor]
...
- [ ] Task: Superconductor - User Manual Verification 'Phase 1: Scoped Runner Contracts' (Protocol in workflow.md)

## Phase 2: Incremental Updater Core
- [ ] Task: Implement `mergeIntoJson<T extends { file: string }>` in `packages/superconductor-core/src/intelligence/incremental-updater.ts`. [TIER-3:TCS=7] [AGENT:caduceus-processor]
...
- [ ] Task: Implement `IncrementalIntelligenceUpdater.update()`. [TIER-4:TCS=11] [AGENT:caduceus-oracle]
...
- [ ] Task: Write unit tests for `IncrementalIntelligenceUpdater`. [TIER-3:TCS=8] [AGENT:caduceus-processor]
...
- [ ] Task: Superconductor - User Manual Verification 'Phase 2: Incremental Updater Core' (Protocol in workflow.md)

## Phase 3: Git Hook & CLI Wrapper
- [ ] Task: Implement `packages/superconductor-core/src/intelligence/cli-update.ts` — thin CLI wrapper. [TIER-3:TCS=5] [AGENT:caduceus-processor]
...
- [ ] Task: Create `scripts/install-git-hook.sh`. [TIER-3:TCS=4] [AGENT:caduceus-processor]
...
- [ ] Task: Update `skills/setup/SKILL.md §2.7` to call `scripts/install-git-hook.sh` after worktrunk install. [TIER-3:TCS=5] [AGENT:caduceus-processor]
...
- [ ] Task: Write integration test for git hook install + incremental update cycle. [TIER-3:TCS=6] [AGENT:caduceus-processor]
...
- [ ] Task: Superconductor - User Manual Verification 'Phase 3: Git Hook & CLI Wrapper' (Protocol in workflow.md)
...
- [ ] Task: Integrate `IntelligenceSnapshotReader` into `skills/implement/SKILL.md`. [TIER-4:TCS=9] [AGENT:caduceus-oracle]
...
- [ ] Task: Integrate into `skills/swarm-orchestrate/SKILL.md §1.1`. [TIER-4:TCS=9] [AGENT:caduceus-oracle]
...
- [ ] Task: Integrate into `skills/standalone-review/SKILL.md`. [TIER-3:TCS=7] [AGENT:caduceus-processor]
    - [ ] Load snapshot; inject `crossCuttingRisk` signal (SAST findings for changed files) into security reviewer context prompt
- [ ] Task: Integrate into `skills/coding-agent/SKILL.md` — Surgical Context Block. [TIER-4:TCS=12] [AGENT:caduceus-oracle]
...
- [ ] Task: Integrate into `skills/new-track/SKILL.md` — Architecture Committee + Plan phases. [TIER-3:TCS=7] [AGENT:caduceus-processor]
...
- [ ] Task: Write integration tests verifying degradation banner output for all 3 snapshot states. [TIER-3:TCS=5] [AGENT:caduceus-processor]
...
- [ ] Task: Superconductor - User Manual Verification 'Phase 4: Universal Skill Integration' (Protocol in workflow.md)

## Phase 5: Observability & Drift Rescan
- [ ] Task: Implement `IntelligenceDriftMonitor` in `packages/superconductor-core/src/intelligence/drift-monitor.ts`. [TIER-3:TCS=6] [AGENT:caduceus-processor]
...
- [ ] Task: Add `DriftMonitor` call to `IntelligenceSnapshotReader.load()` — compute and attach drift state to returned `RepoContext`. [TIER-3:TCS=5] [AGENT:caduceus-processor]
- [ ] Task: Write unit tests for `DriftMonitor`. [TIER-3:TCS=4] [AGENT:caduceus-processor]
...
- [ ] Task: Superconductor - User Manual Verification 'Phase 5: Observability & Drift Rescan' (Protocol in workflow.md)

## Phase X: Integration & Finalization
- [ ] Task: Integrate track 'intelligence_incremental_20260724' into main branch. [TIER-2] [AGENT:caduceus-processor]
- [ ] Task: Verify end-to-end: setup → full scan → commit → hook fires → incremental update → new-track → swarm blueprint uses real TCS. [TIER-4] [AGENT:caduceus-oracle]
